...
import {
  assertionQueryOffset,
  tableQueryOffset,
  incrementalTableOffset,
  linuxDataformCliNotAvailableErrorMessage,
  windowsDataformCliNotAvailableErrorMessage,
} from './constants';
...
export async function getTableSchema(
  ...
  datasetId: string,
  tableId: string
): Promise<{ name: string; metadata: { fullTableId: string } }[]> {
  try {
    await checkAuthentication();
    const bigquery = getBigQueryClient();
    if (!bigquery) {
      vscode.window.showErrorMessage(
        'BigQuery client not available. Please check your authentication.'
      );
      return [];
    }
    const dataset = bigquery.dataset(datasetId, { projectId: projectId });
    const [table] = await dataset.table(tableId).get();
    return table.metadata.schema.fields.map(
...
export function sendNotifactionToUserOnExtensionUpdate(
...
function getTreeRootFromWordInStruct(
  struct: Table[] | Operation[] | Assertion[] | Declarations[],
  searchTerm: string
): string | undefined {
  if (struct) {
...
function updateDependentsGivenObj(
  dependents: Target[],
  targetObjList: Table[] | Assertion[] | Operation[],
  targetToSearch: Target
) {
  ...
  for (let i = 0; i < targetObjList.length; i++) {
    ...
    if (!tableTargets || tableTargets.length === 0) {
      ...
      tableTargets.forEach((tableTarget: Target) => {
        if (
          tableTarget.name === targetToSearch.name &&
          tableTarget.schema === targetToSearch.schema &&
          tableTarget.database === targetToSearch.database
        ) {
...
async function getDependentsOfTarget(
  targetToSearch: Target,
...
export async function getCurrentFileMetadata(
  freshCompilation: boolean
): Promise<CurrentFileMetadata | undefined> {
  ...
  if (!workspaceFolder) {
    return { isDataformWorkspace: false };
  }
  workspaceFolder = `"${workspaceFolder}"`;

  if (freshCompilation || !CACHED_COMPILED_DATAFORM_JSON) {
    let { dataformCompiledJson, errors, possibleResolutions } =
      await runCompilation(workspaceFolder); // Takes ~1100ms
    if (dataformCompiledJson) {
      let fileMetadata = await getQueryMetaForCurrentFile(
        relativeFilePath,
        dataformCompiledJson
      );
      ...
      if (targetToSearch) {
        dependents = await getDependentsOfTarget(
          targetToSearch,
          dataformCompiledJson
        );
      }
...
export async function getPostionOfSourceDeclaration(
  sourcesJsUri: vscode.Uri,
  searchTerm: string
) {
...
export async function getPostionOfVariableInJsFileOrBlock(
...
export async function getTextByLineRange(
...
export async function getTreeRootFromRef(): Promise<string | undefined> {
  ...
  if (!CACHED_COMPILED_DATAFORM_JSON) {
    ...
    let { dataformCompiledJson, errors } = await runCompilation(
      workspaceFolder
    );
    if (dataformCompiledJson) {
...
export async function fetchGitHubFileContent(): Promise<string> {
  ...
  const response = await fetch(
    `https://api.github.com/repos/ashish10alex/${repo}/contents/${filePath}`
  );
...
export function executableIsAvailable(name: string) {
  ...
  try {
    ...
    vscode.window
      .showErrorMessage(`${name} cli not found in path`, 'Installation steps')
      .then((selection) => {
        if (selection === 'Installation steps') {
          vscode.env.openExternal(
            vscode.Uri.parse(
              'https://github.com/ashish10alex/vscode-dataform-tools?tab=readme-ov-file#requirements'
            )
          );
        }
      });
...
export function getFileNameFromDocument(
  ...
  if (!validFileType) {
    ...
    return {
      success: false,
      error: `File type not supported. Supported file types are ${supportedExtensions.join(
        ', '
      )}`,
    };
  }
...
export function getWorkspaceFolder(): string | undefined {
  ...
  if (workspaceFolder === undefined) {
    vscode.window.showWarningMessage(
      `Workspace could not be determined. Please open folder with your dataform project`
    );
    return undefined;
  }
  ...
  vscode.window.showWarningMessage(
    `Not a Dataform workspace. Workspace: ${workspaceFolder} does not have workflow_settings.yaml or dataform.json at its root`
  );
  return undefined;
}
...
export async function writeCompiledSqlToFile(
...
export async function getStdoutFromCliRun(
  exec: any,
  cmd: string
): Promise<any> {
  let workspaceFolder = getWorkspaceFolder();
...
export async function getAllFilesWtAnExtension(
...
export function getDataformActionCmdFromActionList(
  actionsList: string[],
  workspaceFolder: string,
  dataformCompilationTimeoutVal: string,
  includDependencies: boolean,
  includeDownstreamDependents: boolean,
  fullRefresh: boolean
) {
...
export async function getQueryMetaForCurrentFile(
  relativeFilePath: string,
  compiledJson: DataformCompiledJson
): Promise<TablesWtFullQuery> {
  ...
  if (assertions?.length > 0) {
    const assertionsForFile = assertions.filter(
      (assertion) => assertion.fileName === relativeFilePath
    );
    const assertionCountForFile = assertionsForFile.length;
    if (
      assertionCountForFile > 0 &&
  ...
  if (operations?.length > 0 && finalTables.length === 0) {
    ...
    if (operation) {
      ...
      const finalOperationQuery = operation.queries.reduce(
        (acc, query, index) => {
          return acc + `\n -- Operations: [${index + 1}] \n${query}\n`;
        },
        ''
...
export function getDataformCompilationTimeoutFromConfig() {
  let dataformCompilationTimeoutVal: string | undefined = vscode.workspace
    .getConfiguration('vscode-dataform-tools')
    .get('defaultDataformCompileTime');
...
export function getDataformCompilerOptions() {
  let dataformCompilerOptions: string | undefined = vscode.workspace
    .getConfiguration('vscode-dataform-tools')
    .get('compilerOptions');
...
export function getSqlfluffConfigPathFromSettings() {
  ...
  let sqlfluffConfigPath: string | undefined = vscode.workspace
    .getConfiguration('vscode-dataform-tools')
    .get('sqlfluffConfigPath');
...
export function compileDataform(
  workspaceFolder: string,
  isRunningOnWindows: boolean
): Promise<{
  compiledString: string | undefined;
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isRunningOnWindows) {
      ...
      spawnedProcess = spawn(
        ...
        [
          ...
          '--json',
          '--json',
          `--timeout=${dataformCompilationTimeoutVal}`,
        ],
        { shell: true }
      );
    } else {
    ...
    spawnedProcess.on('close', async (code: number) => {
      if (code === 0) {
        resolve({
          compiledString: stdOut,
          errors: undefined,
          possibleResolutions: undefined,
        });
      } else {
        if (stdOut !== '') {
          ...
          graphErrors.forEach(
            (graphError: { message: string; fileName: string }) => {
              errors.push({
                error: graphError.message,
                fileName: graphError.fileName,
              });
            }
          );
          resolve({
            compiledString: undefined,
            errors: errors,
            possibleResolutions: undefined,
          });
        } else {
          let possibleResolutions = [];
          const dataformInstallHint =
            'If using `package.json`, then run `dataform install`';
          if (errorOutput.includes(dataformInstallHint)) {
            ...
            if (_workspaceFolder) {
              ...
              const packageJsonExsists = await vscode.workspace.fs.stat(
                vscode.Uri.file(filePath)
              );
              if (packageJsonExsists) {
                possibleResolutions.push(
                  'run `<b>dataform install</b>` in terminal'
                );
              }
            }
          } else if (
            errorOutput.includes(windowsDataformCliNotAvailableErrorMessage) ||
            errorOutput.includes(linuxDataformCliNotAvailableErrorMessage)
          ) {
            possibleResolutions.push(
              'Run `<b>npm install -g @dataform/cli</b>` in terminal'
            );
          }
          resolve({
            compiledString: undefined,
            errors: [
              {
                error: `Error compiling Dataform: ${errorOutput}`,
                fileName: '',
              },
            ],
            possibleResolutions: possibleResolutions,
...
export async function runCompilation(
  ...
  try {
    let { compiledString, errors, possibleResolutions } = await compileDataform(
      workspaceFolder,
      isRunningOnWindows
    );
    if (compiledString) {
      const dataformCompiledJson: DataformCompiledJson =
        JSON.parse(compiledString);
      CACHED_COMPILED_DATAFORM_JSON = dataformCompiledJson;
      return {
        dataformCompiledJson: dataformCompiledJson,
        errors: errors,
    ...
    return {
      dataformCompiledJson: undefined,
      errors: [{ error: `Error compiling Dataform`, fileName: '' }],
      possibleResolutions: undefined,
    };
  }
}

export async function getDependenciesAutoCompletionItems(
  compiledJson: DataformCompiledJson
) {
  let sourceAutoCompletionPreference = vscode.workspace
    .getConfiguration('vscode-dataform-tools')
    .get('sourceAutoCompletionPreference');
...
export async function getTextForBlock(
  document: vscode.TextDocument,
  blockRangeWtMeta: { startLine: number; endLine: number; exists: boolean }
): Promise<string> {
  ...
  const endPosition = new vscode.Position(
    blockRangeWtMeta.endLine - 1,
    document.lineAt(blockRangeWtMeta.endLine - 1).text.length
  );
...
export async function runMultipleFilesFromSelection(
  workspaceFolder: string,
  ...
  if (
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      if (dataformCompiledJson && relativeFilepath) {
        fileMetadatas.push(
          await getQueryMetaForCurrentFile(
            relativeFilepath,
            dataformCompiledJson.dataformCompiledJson
          )
        );
  ...
  dataformActionCmd = getDataformActionCmdFromActionList(
    actionsList,
    workspaceFolder,
    dataformCompilationTimeoutVal,
    includDependencies,
    includeDownstreamDependents,
    fullRefresh
  );
...
export function handleSemicolonPrePostOps(fileMetadata: TablesWtFullQuery) {
  const preOpsEndsWithSemicolon = /;\s*$/.test(
    fileMetadata.queryMeta.preOpsQuery
  );
  const icrementalPreOpsEndsWithSemicolon = /;\s*$/.test(
    fileMetadata.queryMeta.incrementalPreOpsQuery
  );
  const postOpsEndsWithSemicolon = /;\s*$/.test(
    fileMetadata.queryMeta.postOpsQuery
  );
  ...
  if (
    !icrementalPreOpsEndsWithSemicolon &&
    fileMetadata.queryMeta.incrementalPreOpsQuery !== ''
  ) {
    fileMetadata.queryMeta.incrementalPreOpsQuery =
      fileMetadata.queryMeta.incrementalPreOpsQuery + ';';
...
export async function dryRunAndShowDiagnostics(
  curFileMeta: any,
  queryAutoCompMeta: any,
  document: vscode.TextDocument,
  diagnosticCollection: any,
  showCompiledQueryInVerticalSplitOnSave: boolean | undefined
) {
  ...
  if (showCompiledQueryInVerticalSplitOnSave !== true) {
    showCompiledQueryInVerticalSplitOnSave = vscode.workspace
      .getConfiguration('vscode-dataform-tools')
      .get('showCompiledQueryInVerticalSplitOnSave');
  }

  let currFileMetadata = handleSemicolonPrePostOps(
    queryAutoCompMeta.currFileMetadata
  );

  let queryToDryRun = '';
  if (
    currFileMetadata.queryMeta.type === 'table' ||
    currFileMetadata.queryMeta.type === 'view'
  ) {
    ...
    queryToDryRun =
      currFileMetadata.queryMeta.preOpsQuery +
      currFileMetadata.queryMeta.operationsQuery;
    ...
    let preOpsQuery =
      currFileMetadata.queryMeta.incrementalPreOpsQuery.trimStart();
    if (preOpsQuery && preOpsQuery !== '') {
      preOpsQuery = replaceQueryLabelWtEmptyStringForDryRun(
        currFileMetadata.queryMeta.preOpsQuery
      );
    }
    let incrementalQuery =
      preOpsQuery + currFileMetadata.queryMeta.incrementalQuery.trimStart();
    queryToDryRun = incrementalQuery;
  }

  // take ~400 to 1300ms depending on api response times, faster if `cacheHit`
  let [dryRunResult, preOpsDryRunResult, postOpsDryRunResult] =
    await Promise.all([
  ...
  if (
    dryRunResult.error.hasError ||
    preOpsDryRunResult.error.hasError ||
    postOpsDryRunResult.error.hasError
  ) {
    ...
    if (sqlxBlockMetadata) {
      setDiagnostics(
        document,
        dryRunResult.error,
        preOpsDryRunResult.error,
        postOpsDryRunResult.error,
  ...
  if (!showCompiledQueryInVerticalSplitOnSave) {
    ...
    vscode.window.showInformationMessage(
      `GB: ${dryRunResult.statistics.totalBytesProcessed} - ${combinedTableIds}`
    );
  }
  return [dryRunResult, preOpsDryRunResult, postOpsDryRunResult];
}

export async function compiledQueryWtDryRun(
  document: vscode.TextDocument,
  diagnosticCollection: vscode.DiagnosticCollection,
  showCompiledQueryInVerticalSplitOnSave: boolean
) {
  ...
  dryRunAndShowDiagnostics(
    curFileMeta,
    queryAutoCompMeta,
    document,
    diagnosticCollection,
    showCompiledQueryInVerticalSplitOnSave
  );

  return [
    queryAutoCompMeta.dataformTags,
    queryAutoCompMeta.declarationsAndTargets,
  ];
}
