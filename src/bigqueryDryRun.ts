...
export async function queryDryRun(
  query: string
): Promise<BigQueryDryRunResponse> {
  if (query === '' || !query) {
    return {
      schema: undefined,
      location: undefined,
  ...
  try {
    ...
    return {
      ...
      statistics: {
        totalBytesProcessed: formatBytes(
    ...
    try {
      ...
      const errorLocation = getLineAndColumnNumberFromErrorMessage(
        finalError.message
      );
      return {
        schema: undefined,
